...
def getGraph(contact_map, outdir, no_selfloop=False):
    base_name = 'slm_graph_selfloop'
    g = to_graph(contact_map, norm=True, bisto=True, scale=True)
    if no_selfloop:
        g.remove_edges_from(g.selfloop_edges())
        enum = g.number_of_edges()
        logger.info('After removing selfloop edges, the graph has {} edges now'.format(enum))
        base_name = 'slm_graph_no_selfloop'
    edge_file = os.path.join(outdir, '{}.edges'.format(base_name))
    nx.write_edgelist(g, edge_file, data=['weight'], delimiter='\t')

    return edge_file

def slm_run(work_dir, edge_file, resolution, seed):
    with open(os.path.join(work_dir, 'slm.log'), 'w+') as stdout:
        ...
        n_starts = '10'
        n_iters = '10'
        ofile = os.path.join(work_dir, '{}.slm'.format(basename))
        verb = '1'
        subprocess.check_call(['java', '-jar', app_path('external', 'ModularityOptimizer.jar'), edge_file,
                                ofile, mod_func, str(resolution), opti_algo, n_starts, n_iters, str(seed), verb],
                                stdout=stdout, stderr=subprocess.STDOUT)
    df = pandas.read_csv(ofile, header=None)
    a = int(df.max())+1
    logger.info('Resulted in {} clusters'.format(a))



def getSLMresult(contact_map, ofile):

    def _SLM_read_table(pathname, seq_col=0, cl_col=1):
    ...
    cl_to_ids = _SLM_read_table(ofile, seq_col=None, cl_col=0)
    logger.info('Clustering using SLM resulted in {} clusters'.format(len(cl_to_ids)))
    clustering = {}
...
def write_seqinfo(contact_map, output_dir, clustering, source_fasta=None):
    
    make_dir(output_dir, exist_ok=True)
    # logger.info('Writing seq_info to the path: {}'.format(output_dir))
    ...
    with contextlib.closing(IndexedFasta(source_fasta)) as seq_db:
        ...
        for cl_id, cl_info in clustering.iteritems():
            ...
            cl_path = os.path.join(output_dir, 'seqlist.txt')
            ...
            with open(cl_path, 'a') as output_h:

                logger.debug('Writing seq_info to {}'.format(cl_path))
                ...
                for n, _seq_id in enumerate(np.sort(cl_info['seq_ids']), 1):
                    ...
                    s_name = seq_info[_seq_id].name
                    s_length = seq_info[_seq_id].length
                    s_id = '{0}_{1:0{2}d}'.format('seqID', n, num_width)
                    description = 'contig:{} length:{}'.format(s_name, s_length)
                    output_h.write("{} {}\n".format(s_id, description))

# shortcut ===== end

def cluster_map(contact_map, seed, method, min_len=None, min_sig=None, work_dir='.'):
    """
